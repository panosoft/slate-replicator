...
const eventsTriggerTest = `SELECT * FROM INFORMATION_SCHEMA.triggers WHERE event_object_table = 'events' AND trigger_name = 'events_table_trigger'`;
const eventsTriggerFunctionTest = `SELECT * FROM INFORMATION_SCHEMA.routines WHERE routine_name = 'events_notify_trigger' AND routine_type = 'FUNCTION'`;
...
const replicate = co.wrap(function *(replicationClient) {
	...
	try {
		...
		eventStream = dbUtils.createQueryStream(pooledEventSourceDbClient.dbClient, selectStatement);
		...
		var eventsToCopy = [];
		do {
			var resultFromStream = yield utils.getEventsFromStream(eventStream, _private.maxEventsPerWrite);
...
const doesDatabaseObjectExist = co.wrap(function *(dbClient, testSQLStatement) {
	...
	throw new Error(`Row count (${result.rowCount}) returned "${testSQLStatement}" is invalid for database ${dbClient.database}.  Must be 0 or 1.`);
});

const passedTest = co.wrap(function *(dbClient, testSQLStatement, replicationDatabase) {
	const dbObjectExists = yield doesDatabaseObjectExist(dbClient, testSQLStatement);
...
const validateDatabase = co.wrap(function *(dbClient, replicationDatabase) {
	...
	var passed = yield passedTest(dbClient, eventsTriggerTest, replicationDatabase);
	if (!passed) {
		throw new Error(`${databaseTypeString} database (${dbClient.database}) ${containsString} events table trigger.  ` +
			`Test SQL Statement:  "${eventsTriggerTest})`);
	}
	passed = yield passedTest(dbClient, eventsTriggerFunctionTest, replicationDatabase);
	if (!passed) {
		throw new Error(`${databaseTypeString} database (${dbClient.database}) ${containsString} events table trigger FUNCTION.  ` +
			`Test SQL Statement:  "${eventsTriggerFunctionTest})`);
	}
	passed = yield passedTest(dbClient, insertEventsFunctionTest, replicationDatabase);
	if (!passed) {
	...
	passed = yield passedTest(dbClient, idTableTest, replicationDatabase);
	if (!passed) {
