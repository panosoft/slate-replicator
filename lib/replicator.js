...
const formatInsertStatement = (events) => {
	const eventList = R.reduce((acc, event) => {
		...
		acc[acc.length] = `('${id}', '${eventTimestamp.toISOString()}', '${escapeQuote(JSON.stringify(eventData))}')`;
		return acc;
	}, [], events);
	return 'INSERT INTO EVENTS (id, eventTimestamp, event) VALUES ' + R.join(', ',  eventList);
};
