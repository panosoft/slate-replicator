...
const partitionEvents = (queuedEvents, retrievedEvents) => {
	...
	_private.logger.info({maxts: maxts});
	queuedEvents = R.takeLastWhile(tsEqual, combinedEvents);
	_private.logger.info({combinedEventsLen: combinedEvents.length, queuedEventsLen: queuedEvents.length});
	return {eventsToCopy: R.slice(0, (combinedEvents.length - queuedEvents.length), combinedEvents), queuedEvents: queuedEvents};
};
	...
	try {
		...
		do {
			...
			if (resultFromStream.endOfStream) {
				...
				resultFromPartition = partitionEvents(queuedEvents, resultFromStream.events);
				eventsToCopy = resultFromPartition.eventsToCopy;
				queuedEvents = resultFromPartition.queuedEvents;
			}
			_private.logger.info({eventsToCopyLen: eventsToCopy.length, queuedEventsLen: queuedEvents.length});
