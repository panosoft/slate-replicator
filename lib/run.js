...
const validateReplicationDestinations = (replicationDestinations, eventSource, allErrors) => {
	...
	if (is.array(replicationDestinations)) {
		if (replicationDestinations.length > 0) {
			...
			replicationDestinations.forEach((replicationDestination, i) => {
...
const validateConfigParameters = config => {
	...
	var eventSource = {};
	// optional parameter
	config.maxEventsPerRead = config.maxEventsPerRead || 10000;
	// optional parameter
	config.maxEventsPerWrite = config.maxEventsPerWrite || 10000;
	if (utils.isPositiveInteger(config.maxEventsPerRead)) {
		if (config.maxEventsPerRead < 10000) {
			config.maxEventsPerRead = 10000;
			logger.info(`config.maxEventsPerRead set to minimum value of 10000`);
		}
	}
	else {
		allErrors = R.append('config.maxEventsPerRead is not a positive integer:  ' + config.maxEventsPerRead, allErrors);
	}
...
replicator(config.eventSource, config.replicationDestinations, logger, config.maxEventsPerRead, config.maxEventsPerWrite, config.connectTimeout)
.then(() =>  {
