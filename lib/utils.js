...
const utils = {
	...
	eventColumnsListToInsertValues: eventColumnsList => {
		// eventColumnsList is an array of event objects containing entityId and event properties
		var insertValues = [];
		eventColumnsList.forEach((eventColumns, idx) => insertValues[insertValues.length] = _private.eventColumnsToInsertValues(eventColumns, idx + 1));
		return `$$${insertValues.join(',')}$$`;
	},
	createInsertEventsSQLStatement: insertValuesList => `SELECT insert_events(ARRAY[${insertValuesList}])`,
	unblock: () => {
