...
const _private = {
	eventColumnsToInsertValues: (eventColumns, idx) => {
		...
		idx = idx || 1;
		return `($1[${idx}], $2, '${eventColumns.entityId}', ` + `'${JSON.stringify(eventColumns.event).replace(/'/g, '\'\'')}')`;
	},
	eventColumnsListToInsertValues: eventColumnsObj => {
		// eventColumnsList is an array of event objects containing entityId and event properties
		const eventColumnsList = eventColumnsObj.events;
		var insertValues = R.forEach((eventColumns, idx) => _private.eventColumnsToInsertValues(eventColumns, idx + 1), eventColumnsList);
		return `$$${R.join(',', insertValues)}$$`;
	}
};
...
const utils = {
	...
	// insertValuesList is an array of objects containing an events property that is an array of event objects containing entityId and event properties
	createInsertEventsSQLStatement: insertValuesList => `SELECT insert_events(ARRAY[${R.forEach(_private.eventColumnsToInsertValues, insertValuesList)}])`,
	yieldToEventLoop: () => {
