...
const utils = {
	...
	eventColumnsToInsertValues: (eventColumns, idx) => {
		// id[idx] represents the parameter value for the id column where idx is a 1-based index starting at 1
		idx = idx || 1;
	...
	eventColumnsListToInsertValues: eventColumnsList => {
		var insertValues = [];
		eventColumnsList.forEach((eventColumns, idx) => insertValues[insertValues.length] = utils.eventColumnsToInsertValues(eventColumns, idx + 1));
		return insertValues;
	}
};
